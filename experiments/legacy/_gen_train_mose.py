"""Generator for train_mose_tiny.ipynb"""
import json, os
...
"_KW = {\n"
"    'forge_c': ['calls','function','breaks','affected','import','code','depends'],\n"
...
"            1 + math.cos(math.pi * max(0, step - 50) / (STEPS_M - 50))))\n"
...
out = os.path.join(os.path.dirname(__file__), "train_mose_tiny.ipynb")
...
for i, c in enumerate(nb2["cells"]):
    src = "".join(c["source"])
    total_chars += len(src)
    print(f"  cell {i} ({c['cell_type']:<8}): {len(src):>6} chars")
print(f"Total source: {total_chars:,} chars")
